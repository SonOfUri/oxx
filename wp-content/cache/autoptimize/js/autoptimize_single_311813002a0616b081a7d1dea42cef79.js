...
jQuery(document).ready(function($) {
    $('.b_project img.alignnone, .b_project img.aligncenter').each(function() {
        $(this).parents('p').addClass('added-margin animate');
    });
    $('.motion_js').click(function() {
        var bclss = $(this).attr('data-body');
        if (bclss != '' && bclss != 'undefined') {
            if ($('body').hasClass(bclss)) $('body').removeClass(bclss);
            else $('body').addClass(bclss);
        }
        return false;
    });
    check_fix_menu();
    ...
    $('a[href*=#]').click(function(event) {
        var id = '#' + $(this).attr('href').split('#').slice(-1);
        if ($(id).length > 0) {
            event.preventDefault();
            var top = $(id).offset().top - parseInt($('#header').height()) - 3 * parseInt($('#header').css('padding-top'));
            $('html').animate({
                scrollTop: top
            }, 400);
        }
        $('body.vis_menu').removeClass('vis_menu');
    });
    $(".i_portfolio a").on("click", function(event) {
    ...
    if (navigator.share) {
        $('.i_share.copy.all').on('click', function() {
            navigator.share({
        ...
        $('.i_share.copy.all').removeClass('all');
    }
    $('.i_share').on('click', function() {
        if (!$(this).hasClass('all')) {
            ...
            $('.i_share').addClass('good');
            setTimeout(function() {
                $('.i_share').removeClass('good');
            }, 2000);
        }
    });
    startAnimateElements();

    function startAnimateElements() {
        var $window = $(window),
            $elem = $('.animate');
    ...
    function isScrolledIntoView($elem, $window) {
        ...
        var elemHgh = $elem.height();
        var elemBottom = elemTop + elemHgh;
        return ((elemTop - docViewTop <= wh * 0.94) && (docViewTop <= elemBottom - elemHgh * 0.1));
    }

    function check_fix_menu() {
        let height_header = $('header').height() * 0.5 + parseInt($('header').css('padding-top')) * 0.9;
        if (window.pageYOffset > height_header) $('body').addClass('fixm');
        else $('body').removeClass('fixm');
        if (window.pageYOffset > screen.height) $('.to_up').addClass('act');
        else $('.to_up').removeClass('act');
    }
});