...
let lbwpsInit = function(domUpdate) {
    ...
    for (let i = 0; i < elementorCarouselWidgetList.length; i++) {
        ...
        if (widgetId != null) {
            if (elementorCarouselWidgetList[i].firstElementChild != null && elementorCarouselWidgetList[i].firstElementChild.firstElementChild != null && elementorCarouselWidgetList[i].firstElementChild.firstElementChild.firstElementChild != null && elementorCarouselWidgetList[i].firstElementChild.firstElementChild.firstElementChild.firstElementChild != null) {
    ...
    for (let i = 0; i < elementorImageWidgetList.length; i++) {
        ...
        if (widgetId != null) {
            if (elementorImageWidgetList[i].firstElementChild != null && elementorImageWidgetList[i].firstElementChild.firstElementChild != null) {
    ...
    let openPhotoSwipe = function(element_index, group_index, element, fromURL, returnToUrl) {
        ...
        const options = {
            ...
            pswpModule: () =>
                import ('./lib/photoswipe.esm.min.js'),
            ...
            const captionPlugin = new PhotoSwipeDynamicCaption(lightbox, {
                ...
                captionContent: (slide) => {
                    ...
                    if (slide.data.title && slide.data.title !== '') {
                        caption = caption + '<div class="pswp__caption">' + slide.data.title + '</div>'
                    }
                    if (slide.data.exif && slide.data.exif !== '') {
                        caption = caption + '<div class="pswp__caption__exif">' + slide.data.exif + '</div>'
                    }
                    return caption
                },
        ...
        if (options.desktopSlider) {
            const lbwpsGoTo = (index, animate = !1) => {
            ...
            lightbox.on('uiRegister', () => {
                lightbox.pswp.next = () => lbwpsGoTo(lightbox.pswp.potentialIndex + 1, !0);
                lightbox.pswp.prev = () => lbwpsGoTo(lightbox.pswp.potentialIndex - 1, !0)
            })
        }
        lightbox.init();
        if (lbwpsOptions.hide_scrollbars === '1') {
    ...
    window.lbwpsCopyToClipboard = function(str) {
        ...
        const selected = document.getSelection().rangeCount > 0 ? document.getSelection().getRangeAt(0) : !1;
        el.select();
